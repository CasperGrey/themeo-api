...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Themeo Api account.<br>
        ...
        &mdash; Themeo Api Team
      `
      return sendMail({ toEmail: email, subject: 'Themeo Api - Password Reset', content })
    })
